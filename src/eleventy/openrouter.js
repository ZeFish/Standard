...
export const OpenRouterPlugin = {
  configFunction: (eleventyConfig, options = {}) => {
    const config = {
      ...
      siteUrl: options.siteUrl || process.env.SITE_URL || 'https://standard.ffp.co',
      ...options
    };

    if (!config.enabled) {
      console.log('[Standard AI] Plugin disabled');
      return;
    }

    if (!config.apiKey) {
      console.warn('[Standard AI] Warning: OPENROUTER_KEY not set. AI features will not work.');
    }

    console.log(`[Standard AI] Initializing with model: ${config.model}`);

    // Helper function to call OpenRouter
    const callOpenRouter = async (messages, model = config.model) => {
      if (!config.apiKey) {
      ...
      try {
        ...
        console.error('[Standard AI] Error calling OpenRouter:', error.message);
     ...
     */
    eleventyConfig.addNunjucksAsyncFilter("aiSummarize", async function(content, length = 100, unit = 'words') {
     ...
     */
    eleventyConfig.addNunjucksAsyncFilter("aiTranslate", async function(content, language) {
     ...
     */
    eleventyConfig.addNunjucksAsyncShortcode("aiGenerate", async function(prompt, model) {
     ...
     */
    eleventyConfig.addShortcode("aiChat", function(placeholder = "Ask a question...", model = config.model) {
      return `
...
<script>
  new StandardAI.Search('.ai-search');
      ...
      eleventyConfig.on("eleventy.before", () => {
        ...
        functionFiles.forEach(file => {
          ...
          if (fs.existsSync(src)) {
            ...
            console.log(`[Standard AI] Copied ${file} to functions/api/`);
          }
        });
      });
    }
    ...
    console.log('[Standard AI] Plugin initialized successfully!');
  }
};
