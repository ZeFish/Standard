 ...
 * - Preload internal links with HTMX hx-boost
 * - Add hx-disable to external links
 */
    ...
    visit(tree, "element", (node, index, parent) => {
      if (node.tagName === "table") {
        ...
        if (
          ...
          parent.properties.className.includes(wrapperClass)
        ) {
          return;
        }
    ...
    visit(tree, "element", (node) => {
      if (node.tagName === "a" && node.properties && node.properties.href) {
        ...
        if (isExternal) {
          ...
          // Tell HTMX to ignore this external link
          if (!node.properties["hx-disable"]) {
            node.properties["hx-disable"] = "";
          }
        } else {
          // Internal link - add preload and hx-boost for HTMX navigation
          if (!node.properties.preload) {
            node.properties.preload = true;
          }

          // Add hx-boost="true" for HTMX-enhanced navigation
          if (!node.properties["hx-boost"]) {
            node.properties["hx-boost"] = "true";
          }
        }
      }
    });
