(function () {
  ...
  class StandardLab {
    ...
    injectStyles() {
      ...
      styles.textContent = `
      .standard-lab-badge { font-family: var(--font-interface); position: fixed; width: var(--space); height: var(--space); border-radius: 50%; background: var(--color-accent); border: var(--border); box-shadow: var(--shadow); backdrop-filter: var(--blur); cursor: pointer; z-index: var(--z-toast); line-height:1.5; display: flex; align-items: baseline; justify-content: center; color: var(--color-foreground); transition: all 0.2s ease; padding: 0; margin:0; }
      ...
      .standard-lab-section { margin:0;padding: 6px; border-bottom: var(--border); }
